...
import { type Exercise, getLogsByExercise } from "@/lib/storage"
      ...
      {lastLog && (
        <div className="bg-secondary/50 rounded-xl p-3 space-y-2">
          ...
          <div className="flex items-baseline gap-3">
            <div>
              <span className="text-2xl font-bold text-foreground">{lastLog.weight}</span>
              <span className="text-sm text-muted-foreground ml-1">kg</span>
            </div>
            <div className="text-sm text-muted-foreground">× {lastLog.reps} reps</div>
          </div>
          <div className={`text-sm font-medium ${feelingColors[lastLog.feeling]}`}>
